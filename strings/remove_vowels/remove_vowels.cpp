...
int main() {
    ...
    // size_t is the data type used for the size of a string 
    for (size_t i=0; i<vowels.size(); ++i) { // for each vowel, find every occurance and replace it with a symbol
        ...
        do { // scan until reach the end of the text
            ...
            if (position != string::npos) { // replace character
                size_t index = rand() % symbols.size(); //select a random symbol
                text.at(position) = symbols.at(index);  //replate the character with the symbol
            }
