...
class SocialMediaPost {
  ...
  public:
    ...
    SocialMediaPost& operator=(const SocialMediaPost& source) {
        ...
        if(likedBy != nullptr) {
            delete[] likedBy;
            likedBy = nullptr; // set dangling pointer to null in case source has no likes
        }
        if(source.numLikes > 0) {
            likedBy = new int[source.numLikes];
            for(unsigned int i=0; i<source.numLikes; ++i) {
    ...
    void addLike(int personId) {
        ...
        if(!found) {
            ...
            int* tmpAry = new int[numLikes+1];

            // find the position to insert the new personId
            unsigned int index = 0;
            while(index < numLikes && likedBy[index] < personId) { 
                index++; 
            }

            // copy over everything smaller than the new personId
            for(unsigned int i=0; i<index; ++i) {
                tmpAry[i] = likedBy[i];
            }

            // insert the new personId
            tmpAry[index] = personId;

            // copy over everything else (bigger than the new personId
            for(unsigned int i=index; i<numLikes; ++i) {
                tmpAry[i+1] = likedBy[i];
            }
