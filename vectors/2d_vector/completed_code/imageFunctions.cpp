...
void sepiaImage(vector<vector<Pixel> >& image)
{
    ...
    for(int row=0; row<image.size(); ++row) {
        for(int col=0; col<image.at(row).size(); col++) {
            Pixel new_color;
            new_color.r =   0.393 * image.at(row).at(col).r + 0.768 * image.at(row).at(col).g + 0.189 * image.at(row).at(col).b;
            new_color.g = 0.349 * image.at(row).at(col).r + 0.686 * image.at(row).at(col).g + 0.168 * image.at(row).at(col).b;
            new_color.b =  0.272 * image.at(row).at(col).r + 0.534 * image.at(row).at(col).g + 0.131 * image.at(row).at(col).b;
...
void grayScaleInvertImage(vector<vector<Pixel> >& image) {
    grayscaleImage(image);
    invertImage(image);
}
